...
container_build(){
  ...
  docker run -d --name "$NAME" --restart=always iproyal/pawns-cli:latest -email="$EMAIL" -password="$PASSWORD" -device-name=raspberrypi -device-id=raspberrypi1 -accept-tos >/dev/null 2>&1
